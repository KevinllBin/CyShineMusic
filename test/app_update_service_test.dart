...
import 'package:cy_shine_music/core/services/app_update_service.dart';
  ...
  group('AppUpdateService', () {
    test('parses a valid latest release response', () async {
      final dio = _dioReturning({
        'tag_name': 'v1.10.0',
        'html_url':
            'https://github.com/KevinllBin/CyShineMusic/releases/tag/v1.10.0',
        'name': 'CyShineMusic 1.10.0',
        'draft': false,
        'prerelease': false,
      });
      ...
      expect(release.title, 'CyShineMusic 1.10.0');
      expect(
        release.pageUri,
        Uri.parse(
          'https://github.com/KevinllBin/CyShineMusic/releases/tag/v1.10.0',
    ...
    for (final field in ['draft', 'prerelease']) {
      test('ignores a $field release', () async {
        final dio = _dioReturning({
          'tag_name': 'v2.0.0',
          'html_url':
              'https://github.com/KevinllBin/CyShineMusic/releases/tag/v2.0.0',
          field: true,
        });
        addTearDown(dio.close);
    ...
    test('rejects invalid release fields', () async {
      final dio = _dioReturning({
        'tag_name': 'invalid',
        'html_url':
            'https://github.com/KevinllBin/CyShineMusic/releases/tag/invalid',
      });
      addTearDown(dio.close);
...
class _FakeAdapter implements HttpClientAdapter {
  ...
  Future<ResponseBody> fetch(
    ...
    expect(
      options.uri,
      Uri.parse(
        'https://api.github.com/repos/KevinllBin/CyShineMusic/releases/latest',
      ),
    );
